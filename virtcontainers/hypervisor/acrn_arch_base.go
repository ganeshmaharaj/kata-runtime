...
type acrnArch interface {
	...
	appendConsole(devices []AcrnDevice, path string) []AcrnDevice

	// appendImage appends an image to devices
	appendImage(devices []AcrnDevice, path string) ([]AcrnDevice, error)

	// appendBridges appends bridges to devices
	appendBridges(devices []AcrnDevice) []AcrnDevice

	// appendLPC appends LPC to devices
	// UART device emulated by the acrn-dm is connected to the system by the LPC bus
	appendLPC(devices []AcrnDevice) []AcrnDevice

	// appendSocket appends a socket to devices
	appendSocket(devices []AcrnDevice, socket types.Socket) []AcrnDevice

	// appendNetwork appends a endpoint device to devices
	appendNetwork(devices []AcrnDevice, endpoint Endpoint) []AcrnDevice

	// appendBlockDevice appends a block drive to devices
	appendBlockDevice(devices []AcrnDevice, drive config.BlockDrive) []AcrnDevice
...
type AcrnDevice interface {
	Valid() bool
	AcrnParams(slot int, config *AcrnConfig) []string
}
...
func newAcrnArch(config Config) acrnArch {
	a := &acrnArchBase{
		path:                 acrnPath,
		ctlpath:              acrnctlPath,
...
func (cdev ConsoleDevice) AcrnParams(slot int, config *AcrnConfig) []string {
...
func (netdev NetDevice) AcrnParams(slot int, config *AcrnConfig) []string {
	var acrnParams []string
...
func (blkdev BlockDevice) AcrnParams(slot int, config *AcrnConfig) []string {
	var acrnParams []string
...
func (bridgeDev BridgeDevice) AcrnParams(slot int, config *AcrnConfig) []string {
	var acrnParams []string
...
// AcrnParams returns the acrn parameters built out of this bridge device.
func (lpcDev LPCDevice) AcrnParams(slot int, config *AcrnConfig) []string {
	var acrnParams []string
	var deviceParams []string
...
func (config *AcrnConfig) appendName() {
	if config.Name != "" {
		config.acrnParams = append(config.acrnParams, config.Name)
	}
}

func (config *AcrnConfig) appendDevices() {
	slot := 0
...
func (config *AcrnConfig) appendUUID() {
	if config.UUID != "" {
...
func (config *AcrnConfig) appendACPI() {
	if config.ACPIVirt {
		config.acrnParams = append(config.acrnParams, "-A")
	}
}

func (config *AcrnConfig) appendMemory() {
...
func (config *AcrnConfig) appendCPUs() {
	if config.NumCPU != 0 {
...
func (config *AcrnConfig) appendKernel() {
...
func LaunchAcrn(config AcrnConfig, logger *logrus.Entry) (int, string, error) {
	baselogger = logger
	config.appendUUID()
	config.appendACPI()
	config.appendMemory()
...
func (a *acrnArchBase) appendImage(devices []AcrnDevice, path string) ([]AcrnDevice, error) {
...
func (a *acrnArchBase) appendBridges(devices []AcrnDevice) []AcrnDevice {
...
// appendBridges appends to devices the given bridges
func (a *acrnArchBase) appendLPC(devices []AcrnDevice) []AcrnDevice {
	devices = append(devices,
		LPCDevice{
			Function: 0,
			Emul:     acrnLPCDev,
...
func (a *acrnArchBase) appendConsole(devices []AcrnDevice, path string) []AcrnDevice {
...
func (a *acrnArchBase) appendSocket(devices []AcrnDevice, socket types.Socket) []AcrnDevice {
...
func networkModelToAcrnType(model types.NetInterworkingModel) NetDeviceType {
	switch model {
...
func (a *acrnArchBase) appendNetwork(devices []AcrnDevice, endpoint Endpoint) []AcrnDevice {
...
func (a *acrnArchBase) appendBlockDevice(devices []AcrnDevice, drive config.BlockDrive) []AcrnDevice {
	if drive.File == "" {
		return devices
	}
...
func (a *acrnArchBase) handleImagePath(config Config) {
	if config.ImagePath != "" {
