...
// qemu is an Hypervisor interface implementation for the Linux qemu
type qemu struct {
	id string
...
func newQMPLogger() qmpLogger {
	return qmpLogger{
		logger: logrus.WithField("source", "virtcontainers/hypervisor").WithField("subsystem", "qmp"),
	}
}
...
func (q *qemu) Logger() *logrus.Entry {
	return logrus.WithField("source", "virtcontainers/hypervisor").WithField("subsystem", "qemu")
}
...
func (q *qemu) kernelParameters() string {
	...
	params = append(params, Param{"nr_cpus", fmt.Sprintf("%d", q.maxVCPUs)})
...
// Capabilities add all supported capabilities by QEMU
...
func (q *qemu) setup(id string, hypervisorConfig *Config, vcStore *store.VCStore) error {
...
func (q *qemu) CreateSandbox(ctx context.Context, id string, networkNS NetworkNamespace, hypervisorConfig *Config, store *store.VCStore) error {
...
func (q *qemu) hotplugDevice(devInfo interface{}, devType Device, op Operation) (interface{}, error) {
	switch devType {
	case BlockDev:
...
func (q *qemu) HotplugAddDevice(devInfo interface{}, devType Device) (interface{}, error) {
...
func (q *qemu) HotplugRemoveDevice(devInfo interface{}, devType Device) (interface{}, error) {
...
func (q *qemu) AddDevice(devInfo interface{}, devType Device) error {
	var err error
...
func (q *qemu) GetThreadIDs() (VcpuThreadIDs, error) {
	...
	for _, i := range cpuInfos {
		if i.ThreadID > 0 {
			tid.VCPUs[i.CPU] = i.VcpuThreadID
		}
	}
	return tid, nil
}
...
func (q *qemu) fromGrpc(ctx context.Context, hypervisorConfig *Config, store *store.VCStore, j []byte) error {
	var qp qemuGrpc
